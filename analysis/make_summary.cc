...
class HistogramCollection {
  public:
    ...
    void Fit() {
      ...
      fGausCeren = new TF1((TString)Form("fGausCeren_%s_%dGeV", fName.c_str(), fEnergy), "gaus", fEnergy - 40, fEnergy + 40); 
      fGausCeren->SetLineColor(kBlue);
      fHistCeren->SetOption("p"); 
      fHistCeren->Fit(fGausCeren,"R");
  
      fGausScint = new TF1((TString)Form("fGausScint_%s_%dGeV", fName.c_str(), fEnergy), "gaus", fEnergy - 40, fEnergy + 40); 
      fGausScint->SetLineColor(kRed);
      fHistScint->SetOption("p"); 
      fHistScint->Fit(fGausScint,"R");
  
      fGausSum = new TF1((TString)Form("fGausSum_%s_%dGeV", fName.c_str(), fEnergy), "gaus", 2 * fEnergy - 80, 2 * fEnergy + 80); 
      fGausSum->SetLineColor(kBlack);
      fHistSum->SetOption("p"); 
      fHistSum->Fit(fGausSum,"R");

      fGausDRCorr = new TF1((TString)Form("fGausDRCorr_%s_%dGeV", fName.c_str(), fEnergy), "gaus", fEnergy - 40, fEnergy + 40); 
    ...
    double DualReadoutCorrection(double eC, double eS) { return (eS - 0.221 * eC) / (1 - 0.221); }
    ...
    void Save(TFile* fFile) {
      ...
      fHistCeren->Write();
      if (fIsFitted) fGausCeren->Write();

      fHistScint->Write();
      if (fIsFitted) fGausScint->Write();

      fHistSum->Write();
      if (fIsFitted) fGausSum->Write();
...
void MakeReconstruction(std::string fSuffix, int fEnergy, int fRunNumber, std::vector<std::string> fModule) {
  ...
  for (int i = 0; i < fMaxEvent; i++) {
    ...
    fHistSet_noCut->Fill(fPeakADC_AUX[2], fPeakADC_AUX[3], tEnergyCeren, tEnergyScint, fDWCPOS);

    if (!IsPassingDWC(fDWCPOS))
      continue;

    fHistSet_DWC->Fill(fPeakADC_AUX[2], fPeakADC_AUX[3], tEnergyCeren, tEnergyScint, fDWCPOS);

    if (fPeakADC_AUX[2] < 424.)
      continue;

    fHistSet_PS->Fill(fPeakADC_AUX[2], fPeakADC_AUX[3], tEnergyCeren, tEnergyScint, fDWCPOS);

    if (fPeakADC_AUX[3] > 38.)
      continue;

    fHistSet_MC->Fill(fPeakADC_AUX[2], fPeakADC_AUX[3], tEnergyCeren, tEnergyScint, fDWCPOS);
  }
...
void MakeResolution(std::string fSuffix, std::vector<int> fEnergy) {
  ...
  for (int iEnergy = 0; iEnergy < fEnergy.size(); iEnergy++) {
    ...
    TFile* fFile = new TFile(Form("%se_%dGeV.root", fBaseDir.c_str(), fEnergy.at(iEnergy)), "READ");
